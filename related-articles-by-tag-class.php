...
class RelatedPostsByTag
{
	...
	static function init()
	{
		if (NULL == self::$instance)
			self::$instance = new RelatedPostsByTag();
	}

	function RelatedPostsByTag()
	{
		global $wp_version;
	...
	function admin_head_hook()
	{
		...
		wp_register_script('-by-tag', WP_PLUGIN_URL.'/'.RELATED_ARTICLES_BY_TAG_PARENT_DIR.'/-by-tag.js', array('jquery','suggest'), false);
		wp_localize_script('-by-tag','relatedPostsByTagL10n',
				array(
					'tagsUsed' =>  __('Tags used on this page:'),
					'addTag' => esc_attr(__('Add New Tag')),
				)
		);
		wp_print_scripts('-by-tag');
